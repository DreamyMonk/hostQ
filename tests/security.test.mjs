...
assert.match(setup, /init-admin/, 'setup must initialize admin credentials through the Go panel binary');
...
assert.match(setup, /opcache\.enable=1/, 'setup must enable PHP OPcache by default');
assert.match(setup, /fastcgi_cache_path/, 'setup must prepare optional Nginx FastCGI cache zone');
assert.match(setup, /go build -trimpath/, 'setup must build the Go panel');
assert.match(setup, /hostq-panel\.service/, 'setup must install the Go systemd service');
assert.doesNotMatch(setup, /setup_20\.x|apt-get install .*nodejs|npm ci|npm run build|npm install -g pm2|pm2 start/, 'setup must not install or run the Node/PM2 panel');
...
assert.match(sshUpdate, /api\.github\.com\/repos\/\$\{REPO\}\/tags/, 'SSH updater must be able to resolve the latest tag');
assert.match(sshUpdate, /go build -trimpath/, 'SSH updater must rebuild the Go panel');
assert.match(sshUpdate, /systemctl restart hostq-panel/, 'SSH updater must restart the Go panel service');
assert.doesNotMatch(sshUpdate, /\bnode\b|npm|pm2|panel\.update/, 'SSH updater must not require Node, npm, PM2, or the old helper');
